...
class BoletoData(object):
    ...
    **Parâmetros não obrigatórios**:
    ...
    def _sacador_get(self):
        """Tenta usar o sacador que foi setado ou constroi um
        ...
        """
        if self._sacador is None:
            self.sacador = '%s - %s' % (self.sacador_documento, self.sacador_nome)
        return self._sacador

    def _sacador_set(self, list_sacador):
        self._sacador = list_sacador
        
    sacador = property(_sacador_get, _sacador_set)
